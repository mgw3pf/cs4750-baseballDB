<?php
  ...
  function isAdmin($user_id) {
    global $conn;
    $sql = "SELECT * FROM user_role WHERE id=? AND role_id IS NOT NULL LIMIT 1";
  ...
  function loginById($user_id) {
    global $conn;
    $sql = "SELECT u.id, u.role_id, u.username, r.name as role FROM user_role LEFT JOIN roles r ON u.role_id=r.id WHERE u.id=? LIMIT 1";
...
// Accept a user object, validates user and return an array with the error messages
  function validateUser($user, $ignoreFields) {
      ...
      if (isset($user['passwordOld']) && isset($user['user_id'])) {
        $sql = "SELECT * FROM user_role WHERE id=? LIMIT 1";
      ...
      if (in_array('save_user', $ignoreFields) || in_array('signup_btn', $ignoreFields)) {
        $sql = "SELECT * FROM user_role WHERE email=? OR username=? LIMIT 1";
        $oldUser = getSingleRecord($sql, 'ss', [$user['username']]);
